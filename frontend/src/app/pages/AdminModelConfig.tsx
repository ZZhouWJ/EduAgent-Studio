...
import { modelProviders } from "../data/demoData";
...
export function AdminModelConfig() {
  ...
  const [providers, setProviders] = React.useState(modelProviders);
  const [editing, setEditing] = React.useState(modelProviders[0]);
  const [open, setOpen] = React.useState(false);
  const { toast, showToast } = useInlineToast();

  const filtered = providers.filter((item) => (status === "全部" || item.status === status) && `${item.name}${item.provider}${item.abilities.join("")}`.toLowerCase().includes(query.toLowerCase()));
  const setEnabled = (id: string) => {
    setProviders((items) => items.map((item) => item.id === id ? { ...item, status: item.status === "启用" ? "停用" : "启用" } : item));
    showToast("模型启用状态已更新");
  };

  const stats = [
    { label: "已配置模型", value: `${providers.length}`, hint: "含演示供应商", icon: Bot, tone: "blue" as const },
    { label: "可用模型", value: `${providers.filter((p) => p.status === "启用").length}`, hint: "可被智能体调用", icon: PlugZap, tone: "emerald" as const },
    { label: "今日调用次数", value: "3,480", hint: "峰值 14:00", icon: ActivitySquare, tone: "purple" as const },
    { label: "平均响应时间", value: "512ms", hint: "较昨日 -8%", icon: Gauge, tone: "cyan" as const },
    { label: "异常次数", value: "7", hint: "已进入审计", icon: ToggleLeft, tone: "red" as const },
    { label: "今日成本", value: "¥186", hint: "预算 43%", icon: Coins, tone: "orange" as const },
  ];
  ...
  return (
    <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
      ...
      <section className="edu-card rounded-2xl p-4"><div className="flex flex-wrap items-end gap-4"><SearchInput label="搜索模型、供应商或能力" value={query} onChange={setQuery} /><SegmentedControl value={status} options={["全部", "启用", "观察", "停用"]} onChange={setStatus} /></div></section>
        ...
        {filtered.map((model) => (
          <article key={model.id} className="edu-card edu-card-hover rounded-2xl p-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-400">响应</div><div className="font-black text-slate-900">{model.latency}</div></div>
              ...
              <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-400">成本</div><div className="font-black text-slate-900">{model.cost}</div></div>
            </div>
      ...
      <ModalShell title={`编辑模型配置：${editing.name}`} open={open} onClose={() => setOpen(false)}>
        ...
        <div className="mt-5 flex justify-end gap-3"><button onClick={() => setOpen(false)} className={secondaryButton}>取消</button><button onClick={() => { setOpen(false); showToast("模型配置已保存"); }} className={primaryButton}><Save className="h-4 w-4" />保存配置</button></div>
      </ModalShell>
      {toast}
    </div>
  );
