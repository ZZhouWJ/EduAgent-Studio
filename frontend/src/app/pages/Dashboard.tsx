...
const STATS = [
  ...
  { label: "学生数", value: "128", change: "+12", hint: "画像持续更新", icon: Users, color: "text-slate-700", bg: "bg-slate-100", ring: "ring-slate-200" },
  { label: "学习资源", value: "246", change: "+34", hint: "讲义/题库/案例", icon: Library, color: "text-emerald-700", bg: "bg-emerald-50", ring: "ring-emerald-100" },
  { label: "资源生成调用", value: "1,284", change: "+156", hint: "近 7 天", icon: Bot, color: "text-slate-700", bg: "bg-slate-100", ring: "ring-slate-200" },
  { label: "平均掌握度", value: "76%", change: "+2.4%", hint: "课程目标达成", icon: BarChart2, color: "text-orange-700", bg: "bg-orange-50", ring: "ring-orange-100" },
  { label: "审核通过率", value: "92%", change: "+1.2%", hint: "教师审核通过", icon: CheckCircle2, color: "text-cyan-700", bg: "bg-cyan-50", ring: "ring-cyan-100" },
];

const FLOW_STEPS = [
  { title: "学生画像", desc: "六维特征随学随新", icon: Users, tone: "slate" },
  { title: "诊断与定位", desc: "识别薄弱点与目标", icon: BrainCircuit, tone: "blue" },
  { title: "路径规划", desc: "生成依赖学习路径", icon: Route, tone: "emerald" },
  { title: "资源生成", desc: "讲义、题库、案例、脚本", icon: Layers3, tone: "orange" },
  { title: "教师审核", desc: "证据追溯与风险把关", icon: ShieldCheck, tone: "cyan" },
  { title: "学习反馈", desc: "测评驱动画像更新", icon: FileCheck2, tone: "emerald" },
];
...
export function Dashboard() {
  return (
    <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
      <section className="edu-card relative overflow-hidden rounded-2xl bg-white">
        <div className="relative grid min-h-[210px] grid-cols-[1.28fr_0.72fr] gap-8 p-8">
          <div className="flex min-w-0 flex-col justify-center">
            ...
            <h2 className="text-[32px] font-semibold leading-tight text-slate-900">
              智学工坊 <span className="text-slate-700">EduAgent Studio</span>
            </h2>
            <p className="mt-2 text-lg font-medium text-slate-700">
              面向高校专业课程的个性化学习资源生成平台
            </p>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                to="/agent-workbench"
                className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 text-sm font-semibold text-white transition-colors hover:bg-slate-800"
              >
                进入工作台
              ...
              <Link
                to="/student-profile"
                className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50"
              >
                查看学生画像
              </Link>
            </div>
          ...
          <div className="relative hidden min-h-[184px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-5 xl:block">
            <div className="relative flex h-full flex-col justify-between">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">资源生成链路</div>
                  <div className="mt-1 text-lg font-semibold text-slate-900">可信资源生成闭环</div>
                </div>
                <div className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700">
                  <Network className="h-5 w-5" />
                </div>
              </div>

              <div className="relative mt-4 grid grid-cols-3 gap-3">
                <div className="absolute left-[16%] right-[16%] top-1/2 h-px bg-slate-200" />
                {[
                  { label: "画像", icon: Users, cls: "text-slate-700 bg-white ring-slate-200" },
                  { label: "生成", icon: Bot, cls: "text-slate-700 bg-white ring-slate-200" },
                  { label: "资源", icon: Library, cls: "text-slate-700 bg-white ring-slate-200" },
                ].map((node) => {
                  const Icon = node.icon;
                  return (
                    <div key={node.label} className="relative rounded-xl border border-slate-200 bg-white p-3 text-center">
                      <div className={`mx-auto grid h-10 w-10 place-items-center rounded-lg ring-1 ${node.cls}`}>
                        <Icon className="h-5 w-5" />
                      </div>
                      <div className="mt-2 text-sm font-semibold text-slate-800">{node.label}</div>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2">
                <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                  ...
                  <div className="text-base font-semibold text-slate-900">82%</div>
                </div>
                <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                  <div className="text-[11px] text-slate-500">低风险</div>
                  <div className="text-base font-semibold text-slate-900">94%</div>
                </div>
                <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
      ...
      <section className="edu-card rounded-2xl p-6">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">学习闭环流程</h3>
