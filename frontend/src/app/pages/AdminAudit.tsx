...
export function AdminAudit() {
  ...
  const [selected, setSelected] = React.useState<(typeof auditLogs)[number] | null>(null);
  const { toast, showToast } = useInlineToast();
  const filtered = auditLogs.filter((log) => (risk === "全部" || log.status === risk || log.safety === risk) && `${log.user}${log.agent}${log.model}`.toLowerCase().includes(query.toLowerCase()));
  const stats = [
    { label: "今日调用", value: "3,480", hint: "智能体与模型", icon: ActivitySquare, tone: "blue" as const },
    { label: "成功率", value: "96.4%", hint: "近 24 小时", icon: ShieldAlert, tone: "emerald" as const },
    { label: "平均响应时间", value: "512ms", hint: "P95 1.8s", icon: Timer, tone: "purple" as const },
    { label: "Token 消耗", value: "1.84M", hint: "输入输出合计", icon: Gauge, tone: "cyan" as const },
    { label: "风险调用", value: "7", hint: "高频与低可信", icon: ShieldAlert, tone: "red" as const },
    { label: "今日成本", value: "¥186", hint: "预算可控", icon: WalletCards, tone: "orange" as const },
  ];
  return (
    <PageShell>
      ...
      <section className="edu-card rounded-2xl p-4"><div className="flex flex-wrap items-end gap-4"><SearchInput label="搜索用户、智能体或模型" value={query} onChange={setQuery} /><SegmentedControl value={risk} options={["全部", "成功", "风险", "高频", "通过"]} onChange={setRisk} /></div></section>
      ...
      {selected && <DetailDrawer title="调用详情" subtitle={`${selected.user} / ${selected.agent}`} open={!!selected} onClose={() => setSelected(null)}>
        {[
          ["请求摘要", "学生或教师围绕课程薄弱点触发智能体调用，输入包含画像、任务和课程证据。"],
          ["输出摘要", "模型返回结构化学习资源、引用来源、风险提示和下一步建议。"],
          ["证据来源", `知识库命中 ${selected.hit}，引用第 6 章事务与并发控制。`],
