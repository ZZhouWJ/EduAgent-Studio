...
function mapAgent(a: { id: string; name: string; description: string; type: string }) {
  return {
    ...
    name: a.name,
    model: a.type,
    prompt: a.description.length > 20 ? a.description.slice(0, 20) + "…" : a.description,
    duty: a.description,
    enabled: true,
    calls: "-",
    duration: "-",
...
export function AdminAgentConfig() {
  ...
  const [editing, setEditing] = React.useState<ReturnType<typeof mapAgent> | null>(null);
  const [open, setOpen] = React.useState(false);
  const { toast, showToast } = useInlineToast();

  const agentsState = useApi(() => agentsApi.listAgents(), []);

  const agents = ((agentsState.data ?? []) as Array<{ id: string; name: string; description: string; type: string }>).map(mapAgent);
  const filtered = agents.filter((agent) => `${agent.name}${agent.model}${agent.prompt}${agent.duty}`.toLowerCase().includes(query.toLowerCase()));

  const stats = [
    { label: "智能体数量", value: `${agents.length || "-"}`, hint: "核心链路", icon: Bot, tone: "blue" as const },
    { label: "启用智能体", value: `${agents.filter((a) => a.enabled).length || "-"}`, hint: "可执行", icon: CheckCircle2, tone: "emerald" as const },
    { label: "工作流数量", value: "4", hint: "教学主流程", icon: GitBranch, tone: "purple" as const },
    { label: "今日执行次数", value: "-", hint: "待接入统计", icon: ActivitySquare, tone: "cyan" as const },
    { label: "平均成功率", value: "-", hint: "待接入监控", icon: ShieldCheck, tone: "orange" as const },
    { label: "异常次数", value: "-", hint: "待接入审计", icon: Clock3, tone: "red" as const },
  ];

  const toggle = (id: string) => {
    showToast("智能体启用状态：TODO - 后端无 toggle 接口");
  };
  ...
  return (
    <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
      ...
      {agentsState.loading ? (
        ...
        <div className="flex items-center justify-center py-16 text-sm text-slate-400">暂无智能体数据</div>
      ) : (
            ...
            {filtered.map((agent) => (
              <article key={agent.id} className="edu-card edu-card-hover rounded-2xl p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-slate-50 p-3"><div className="text-xs text-slate-400">类型</div><div className="font-black text-slate-900">{agent.model}</div></div>
      ...
      <ModalShell title={editing ? `编辑智能体：${editing.name}` : "编辑智能体"} open={open} onClose={() => setOpen(false)}>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3"><button onClick={() => setOpen(false)} className={secondaryButton}>取消</button><button onClick={() => { setOpen(false); showToast("智能体配置已保存（编辑功能 TODO）"); }} className={primaryButton}><Save className="h-4 w-4" />保存</button></div>
        </div>
      </ModalShell>
      {toast}
    </div>
