...
export function AdminLogs() {
  ...
  const [selected, setSelected] = React.useState<(typeof operationLogs)[number] | null>(null);
  const { toast, showToast } = useInlineToast();
  const filtered = operationLogs.filter((log) => (type === "全部" || log.type === type) && (risk === "全部" || log.risk === risk) && `${log.actor}${log.object}${log.ip}`.toLowerCase().includes(query.toLowerCase()));
  const stats = [
    { label: "今日日志", value: "1,284", hint: "持续写入", icon: FileClock, tone: "blue" as const },
    { label: "登录次数", value: "386", hint: "含学生端", icon: LogIn, tone: "cyan" as const },
    { label: "资源操作", value: "428", hint: "生成与审核", icon: ActivitySquare, tone: "purple" as const },
    { label: "审核操作", value: "92", hint: "教师复核", icon: ShieldAlert, tone: "emerald" as const },
    { label: "配置变更", value: "18", hint: "模型与模板", icon: Settings2, tone: "orange" as const },
    { label: "异常操作", value: "3", hint: "已拦截", icon: ShieldAlert, tone: "red" as const },
  ];
  return (
