...
| 角色 | 用户名 | 演示定位 |
| --- | --- | --- |
| 管理员 | `admin` | 用户、模型、成本、审计与平台治理 |
| 教师 | `teacher_li` | 3 门课程、学生分析、知识库、任务与资源审核 |
| 学生 | `student_zhang` | 数据库初阶，重点演示薄弱点诊断与逾期任务 |
| 学生 | `student_liu` | Python 中阶，重点演示代码实践与已完成任务 |
| 学生 | `student_chen` | 软件工程进阶，重点演示项目协作与质量复盘 |

`DEMO_PASSWORD` 至少 12 个字符，不能使用示例占位值。生产部署不得创建这些开发种子账号。

脚本会构造完整且可重复的演示链路：3 门课程、19 个知识点与资料分块、3 份差异化画像和掌握度、学习任务及学生进度、AI 资源的草稿/待审/通过/驳回状态、资源审核与原文证据、学习反馈、画像对话、Tutor 会话、协作项目、AI 调用成本和审计日志。已知的旧样例账号会被停用，密码不会写入 SQL、前端或版本库。
