...
| 角色 | 用户名 | 密码 | 演示定位 |
| --- | --- | --- | --- |
| 管理员 | `admin` | `EduAgent@Demo2026#A3` | 用户、模型、成本、审计与平台治理 |
| 教师 | `teacher_li` | `EduAgent@Demo2026#A3` | 3 门课程、学生分析、知识库、任务与资源审核 |
| 学生 | `student_zhang` | `EduAgent@Demo2026#A3` | 数据库初阶，重点演示薄弱点诊断与逾期任务 |
| 学生 | `student_liu` | `EduAgent@Demo2026#A3` | Python 中阶，重点演示代码实践与已完成任务 |
| 学生 | `student_chen` | `EduAgent@Demo2026#A3` | 软件工程进阶，重点演示项目协作与质量复盘 |

当前比赛演示环境统一使用表中的共享密码。重新构造演示数据时，将该密码作为 `DEMO_PASSWORD` 注入种子脚本。此密码已公开记录在仓库中，仅限本地或隔离的比赛演示环境；任何联网或生产部署都必须更换密码，并且不得创建这些开发种子账号。

脚本会构造完整且可重复的演示链路：3 门课程、19 个知识点与资料分块、3 份差异化画像和掌握度、学习任务及学生进度、AI 资源的草稿/待审/通过/驳回状态、资源审核与原文证据、学习反馈、画像对话、Tutor 会话、协作项目、AI 调用成本和审计日志。已知的旧样例账号会被停用；密码仅记录在本节，不会硬编码进 SQL、种子脚本或前端。
